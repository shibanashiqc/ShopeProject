				...
				<form class="form-horizontal" enctype="multipart/form-data">
						...
						<div class="md:flex flex-row md:space-x-4 w-full text-xs">
							<div class="mb-3 space-y-2 w-full text-xs">
								...
								<input wire:model='pdname' placeholder="Product Name" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg h-10 px-4" required="required" type="text" name="integration[shop_name]" id="integration_shop_name">
    ...
    @error('feautured')  <p class="text-danger">{{$message}}</p> @enderror
                ...
                @error('categoryy')  <p class="text-danger">{{$message}}</p> @enderror
								...
								<div class="mt-5 text-right md:space-x-3 md:block flex flex-col-reverse">
									...
									<button wire:click.prevent='addProduct' class="mb-2 md:mb-0 bg-green-400 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg hover:bg-green-500">
                                        <i wire:loading wire:target="addProduct" class="fa fa-spinner fa-spin" style="font-size:20px"></i> &nbsp;Save</button>
								</div>
